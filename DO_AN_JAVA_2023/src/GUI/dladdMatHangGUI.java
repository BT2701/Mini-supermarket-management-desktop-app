...
////                    MatHangDTO mh=new MatHangDTO(Mamh, tenmh, sogiamua, sogiaban, ngaysx, hansudung, ERROR, ALLBITS, ngaynhap, ABORT, malh, madvt, img)
//                    mathangBUS.add(mh);
...
//                        
//                         temp2=file.toString().substring(56, file.toString().length());
//                        
...
//		this.setVisible(false);
	...
	private JLabel title, btnadd, btnimg;
	private JLabel txtmamh, txtten, txtgiamua, txtgiaban, txtngaysanxuat, txthansudung, txtslnhap, txtslban,
			txtngaynhap, txtvat, txtmalh, txtmadvt;
	...
	public void init() {
		...
		btnadd.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				String madvttemp = cmbdvt.getSelectedItem().toString();
				try {
					if (tenmh.trim().isEmpty() || giamua.trim().isEmpty() || giaban.trim().isEmpty()
							|| slnhap.trim().isEmpty() || vat.trim().isEmpty() || malhtemp.equalsIgnoreCase("Chọn loại")
							|| madvttemp.equalsIgnoreCase("Chọn ĐVT")) {
						{
				...
				String img = temp2;
				MatHangDTO mh = new MatHangDTO(Mamh, tenmh, sogiamua, sogiaban, ngaysx, hansudung, soslnhap, 0,
						ngaynhap, sovat, malh, madvt, img);
				if (!tenmh.trim().isEmpty() && tbten.getText() == null && tblh.getText() == null
						&& tbdvt.getText() == null && !tenmh.trim().isEmpty() && !giaban.trim().isEmpty()
						&& !giamua.trim().isEmpty() && !slnhap.trim().isEmpty() && !vat.trim().isEmpty()
						&& tbgiaban.getText() == null && tbgiamua.getText() == null && tbslnhap.getText() == null
						&& tbvat.getText() == null && !madvt.equalsIgnoreCase(null) && !malh.equalsIgnoreCase(null)) {
					mathangBUS.add(mh);
					cleanView();
